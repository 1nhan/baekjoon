...
#include<iostream>
using namespace std;
constexpr int member = 30;
int main()
{
	bool class_member_check[member + 1] = { false }; // 0부터인 인덱스를 1로 당김

	int num = 0;
	for (int i = 0; i < member - 2; i++)
	{
		cin >> num;
		if (num < 1 || member < num) return 0;
		class_member_check[num] = true;
	}
	for (int i = 1; i <= member; ++i)
		if (!class_member_check[i])
			cout << i << '\n';
	return 0;
}
